...
server <- function(input, output, session) {
        ...
        card_body(
          ...
          tags$div(
            ...
            tags$ul(class = "mb-0",
              ...
              tags$li("Validation: 359 automated and 49 manual tests (was 338 and 44), with a new section for ",
          ...
          tags$div(
            ...
            tags$p(class = "mb-1", tags$strong("Results can differ from v1.3.0"),
                   ...
                   "with profiles whose terminal fit is below the minimum R\u00b2 (half-life and derived parameters are now missing), with a replicate design, with treatments not named \"Reference\" and \"Test\", or with a confidence limit that rounds onto an acceptance limit, and for sample sizes of scaled methods. See the fixes below."),
            tags$ul(class = "mb-0",
              ...
              tags$li(tags$strong("Correctness fix: "), "steady state: the dosing interval \u03C4 was guessed as last minus first sampling time, and AUC\u03C4 was AUC to the last sample. With a missing concentration at \u03C4 (for example BLQ) average concentration was too low, and without a pre-dose sample \u03C4 was wrong. You now enter \u03C4; AUC\u03C4 runs from 0 to \u03C4 (extrapolated when needed), CL/F and Vz/F use AUC\u03C4, and average concentration, trough, fluctuation and swing are reported in all paths, with AUC\u03C4 compared in Bioequivalence"),
              tags$li(tags$strong("Correctness fix: "), "Plan a Study used an expected ratio of 95% for every method; the default now follows the method as in PowerTOST (90% for highly variable drugs, 97.5% for narrow therapeutic index drugs), which changes sample sizes considerably, and parallel designs ask for the total CV"),
              tags$li(tags$strong("Correctness fix: "), "the minimum R\u00b2 for half-life was not applied to the results: half-life, AUC to infinity, CL/F, Vz/F and MRT were reported from any automatic fit, however poor, although the Methods and the Analysis Record said such fits were rejected. They are now left empty, with a note, unless the points were selected by hand"),
              tags$li(tags$strong("Correctness fix: "), "the Half-Life Review could show a different automatic fit (other points and half-life) than the one used in the results. It now shows NonCompart's own fit"),
              tags$li(tags$strong("Correctness fix: "), "in One Subject at a Time, switching to another profile kept the previous result, so the result card, CSV and Analysis Record could combine one subject's parameters with another subject's data. Results are now cleared when the profile or the data change; batch and bioequivalence results are also cleared when new data are processed"),
              tags$li(tags$strong("Correctness fix: "), "the bioequivalence analysis took the alphabetically first treatment as the Reference unless it was named \"Reference\", so with names such as New/Old or A/B the ratio and confidence interval could be inverted. You now choose the Reference treatment (it is suggested only for unambiguous names such as R or Ref), and the results table shows which treatments were compared"),
              tags$li(tags$strong("Correctness fix: "), "Plan a Study ignored the Test product's variability for the EMA and FDA scaled methods and for narrow therapeutic index drugs, and used the Reference CV for both; sample sizes could be several times too small or too large. Both CVs are now used"),
              tags$li(tags$strong("Correctness fix: "), "\"Use CV from my PK analysis\" filled the within-subject CV with the spread of Cmax across subjects, which includes the differences between subjects and inflates the sample size. It now offers the within-subject CV from the bioequivalence analysis"),
              tags$li(tags$strong("Correctness fix: "), "in the batch analysis of crossover data, dose-normalised parameters (for example Cmax/Dose) were divided by other subjects' doses. Each profile now uses its own subject's dose"),
              ...
              tags$li(tags$strong("Correctness fix: "), "the bioequivalence comparison accepted widened acceptance limits without checking that the point estimate lies within 80.00\u2013125.00%. This is now required by default for limits wider than 80\u2013125%; untick it only for comparisons that have no such rule, such as drug-interaction no-effect boundaries"),
  ...
  output$dl_example_parallel <- downloadHandler(
  ...
  output$dl_example_adnca <- downloadHandler(
    filename = function() "example_adnca.csv",
    content = function(file) file.copy("data/example_adnca.csv", file)
  )
