...
**Status:** implemented on branch `claude/partial-auc-handoff-a2cad0`, not merged. Phases 1–6 are done, including the whole documentation pass of §7: app text, Data Guide, URS (62 requirements), IQ/OQ/PQ (section 3.17, regenerated by `validation/make_iqoqpq.py`), user manual v1.6 (docx and PDF), READMEs and APP_VERSION 1.5.0. Validation: 358 automated tests (section PAUC, 20 tests) and 49 manual tests, all passing.
