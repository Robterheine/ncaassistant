...
### 4.4 Phase 4 decision and implementation record (2026-09-17)

**Decision (with the maintainer): option B, parameter codes only.** No PP-shaped
table or PP export; instead the official CDISC code and name of each reported
parameter, from one pinned and clearly stated release. The maintainer asked
that the app and the reports say which standard was used.

- Source: CDISC SDTM Controlled Terminology, **release 2026-03-27**, downloaded
  from NCI EVS on 2026-09-17 (`SDTM Terminology.txt`; the release date is the
  `FileOID`/`AsOfDateTime` of the matching ODM file; SHA-256 recorded in
  `cdisc/ct_release.dcf`). Codelists C85839 (PKPARMCD → PPTESTCD) and C85493
  (PKPARM → PPTEST), paired by NCIt code.
- `cdisc/pk_parameter_map.csv` maps each app parameter to a code **by route and
  steady state**: at steady state NonCompart calculates CL/F, Vz/F, CL and Vz
  from AUClast, so they map to CLFTAU, VZFTAU, CLTAU, VZTAU; intravascular MRT
  maps to MRTIB* (bolus) or MRTIC* (infusion); `*_DN` to the "Norm by Dose"
  codes; `CMIN_SS` → CMIN and `FLUCTP` → FLUCP (the §3.3 corrections). No code
  exists for CLSTP (predicted Clast), b0 (intercept) or steady-state VSSO; they
  are listed with that reason. TAU, AUCTAU and CAVG carry a note that the app
  takes the sampled interval as the dosing interval.
- `cdisc/pk_parameter_terms.csv` holds the 63 pinned terms (code, name, NCIt
  code, definition), generated by `cdisc/extract_pk_terms.R`, which refuses a
  map entry whose code is not in the release.
- Shown in: a collapsible "CDISC parameter codes (SDTM Controlled Terminology
  <release>)" table under the batch, BE and single-subject results; a
  `CDISC_Parameter_Codes` sheet (release statement in its first rows) in the
  batch and BE Excel downloads and in every Analysis Record's `results.xlsx`;
  `cdisc_terminology` in the settings JSON; a statement in the HTML summary; a
  "Standards and Terminology" card on the About page. Every place states that
  this is a code lookup, not an SDTM PP dataset, with no conformance claim.
- The circular `cdisc_pk_names()` stub is removed. EXP-CD-01 now checks the
  pinned release; EXP-CD-02 route/steady-state mapping; EXP-CD-03 that every
  NonCompart output (3 routes × steady state) has a code or an explicit reason;
  REC-09 the record contents. Validation 296/296.
- **Maintenance:** when updating, download the current release, run
  `extract_pk_terms.R` with its release date, and review the map.
